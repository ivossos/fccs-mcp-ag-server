...
class DCFValuation:
    def __init__(self):
        # Base Year (FY24) Financial Data from FCCS
        self.base_revenue = 649457033.76
        self.base_ebitda = 34484682.88
        self.base_operating_income = 41417367.06
        self.base_net_income = -9638049.91
        self.cash = 3194819.01
        self.capex = 6906346.26
        ...
        self.ebitda_margin = self.base_ebitda / self.base_revenue  # ~5.3%
        self.capex_pct_y1_3 = 0.011       # 1.1% of revenue
        self.capex_pct_y4_5 = 0.010      # 1.0% of revenue
    ...
    def calculate_fcf(self, ebitda, capex, tax_rate):
        ...
        # Simplified: assuming EBITDA approximates operating cash flow
    ...
    def generate_report(self):
        ...
        print("\n1. BASE YEAR FINANCIAL METRICS (FY24)")
        print("-" * 80)
        ...
        print("-" * 80)
        final_fcf = projections[-1]['FCF']
        print(f"Final Year FCF (FY29):       ${final_fcf:>20,.2f}")
        print(f"Terminal Growth Rate:        {self.terminal_growth*100:>19.2f}%")
...
if __name__ == "__main__":
    ...
    projections_df.to_csv('dcf_projections.csv', index=False)
    print("\nProjections exported to: dcf_projections.csv")
    
    # Export sensitivity analysis
    sensitivity_df = dcf.sensitivity_analysis()
    sensitivity_df.to_csv('dcf_sensitivity_analysis.csv', index=False)
    print("Sensitivity analysis exported to: dcf_sensitivity_analysis.csv")
