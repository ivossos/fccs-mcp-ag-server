"""Comprehensive Financial Analysis for FY25 YTD.
...
async def get_intercompany_matching_status() -> Dict:
    """Get intercompany matching report status."""
    try:
        result = await generate_intercompany_matching_report(
            parameters={
                "scenario": "Actual",
                "year": "FY25",
                "period": "Dec"
...
async def generate_comprehensive_analysis():
    ...
    try:
        ...
        print("Generating intercompany matching report...")
        ic_status = await get_intercompany_matching_status()
        
        if ic_status.get("status") == "success":
            print("[OK] Intercompany matching report generated successfully")
            data = ic_status.get("data", {})
            
            # Try to extract key information
            if isinstance(data, dict):
                print("\nReport Summary:")
                for key, value in data.items():
                    if key not in ["status", "links"]:
                        print(f"  {key}: {value}")
            elif isinstance(data, str):
                print(f"\nReport: {data[:500]}...")  # Show first 500 chars
            else:
                print(f"\nReport data available: {type(data)}")
        else:
            error = ic_status.get("error", "Unknown error")
            print(f"[WARNING] Could not generate intercompany matching report: {error}")
